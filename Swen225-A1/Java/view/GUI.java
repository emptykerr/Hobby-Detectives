...
//      playButton.setPreferredSize(new Dimension(getWidth()/7, getHeight()/7));
...
//        quitButton.setPreferredSize(new Dimension(getWidth()/7, getHeight()/7));
    ...
    private void selectCharacters() {
        ...
        if (game.getPlayerCount() == chosenAmountOfPlayers) {
            ...
            dispose();
            Ongoing ongoing = new Ongoing(game);
            game.setOngoingView(ongoing); // give the game access to the ongoing view
            game.loop();
